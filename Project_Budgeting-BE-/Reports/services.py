...
def get_all_tab_data(filters):
    ...
    if user:
        is_admin_or_manager = user.is_superuser or user.roles.filter(role_name__in=["Admin", "Manager", "Project Manager"]).exists()
...
def get_project_tab_data(filters):
    ...
    for pb in qs:
        ...
        data.append({
            ...
            "budget": pb.total_budget or (pb.project.created_from_quotation.total_amount if pb.project.created_from_quotation else None) or Decimal("0.00"),
            "invoiced": invoiced,
            "received": received,
            "expenses": expenses,
            "profit": received - expenses,
...
def get_payment_tab_data(filters):
    ...
    if user:
        is_admin_or_manager = user.is_superuser or user.roles.filter(role_name__in=["Admin", "Manager", "Project Manager"]).exists()
        if not is_admin_or_manager:
            qs = qs.filter(invoice__project__project_manager=user)
