...
class PointOfContactSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # Check global uniqueness of poc_mobile
        poc_mobile = full_number
        instance = self.instance

        queryset = POC.objects.filter(poc_mobile=poc_mobile)
        if instance:
            queryset = queryset.exclude(pk=instance.pk)

        if queryset.exists():
            raise serializers.ValidationError({
                "poc_mobile": "A point of contact with this mobile number already exists."
            })

        # Check uniqueness of poc_email for this company
        company = attrs.get("company") or (instance and instance.company)
        poc_email = attrs.get("poc_email")

        if company and poc_email:
            email_queryset = POC.objects.filter(company=company, poc_email__iexact=poc_email)
            if instance:
                email_queryset = email_queryset.exclude(pk=instance.pk)
            if email_queryset.exists():
                raise serializers.ValidationError({
                    "poc_email": "A point of contact with this email address already exists for this company."
                })
