...
class ProjectBudgetAPIView(APIView):
    ...
    def get(self, request, project_no):
        try:
            budget = ProjectBudget.objects.select_related("project").get(project__project_no=project_no)
            is_admin_or_manager = request.user.is_superuser or request.user.roles.filter(role_name__in=["Admin", "Manager", "Project Manager"]).exists()
            if not is_admin_or_manager and budget.project.project_manager != request.user:
                return Response(
                    {"error": "You do not have permission to access this budget"},
                    status=status.HTTP_403_FORBIDDEN
                )
        except ProjectBudget.DoesNotExist:
            return Response(
...
class ProjectNamesAPIView(APIView):
    ...
    def get(self, request):
        ...
        project_names = list(qs.values_list('project_name', flat=True))
