    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-800">
                    <p className="text-xs text-gray-500 mb-1 dark:text-gray-400">Allocated</p>
                    <p className="text-xl font-bold text-gray-900 dark:text-white">{formatHoursHM(totals.allocated)}</p>
                </div>
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-800">
                    <p className="text-xs text-gray-500 mb-1 dark:text-gray-400">Consumed (live)</p>
                    <p className="text-xl font-bold text-gray-900 dark:text-white">{formatHoursHM(totals.consumed)}</p>
                </div>
                <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 dark:bg-gray-800 dark:border-gray-800">
                    <p className="text-xs text-gray-500 mb-1 dark:text-gray-400">Remaining</p>
                    <p className="text-xl font-bold text-gray-900 dark:text-white">{formatHoursHM(totals.remaining)}</p>
                </div>
            </div>

            <div className="border border-gray-200 rounded-xl overflow-hidden dark:border-gray-800">
                <div className="grid grid-cols-12 gap-4 px-4 py-3 bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-500 uppercase dark:bg-gray-800 dark:border-gray-800 dark:text-gray-400">
                ...
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                    {tasks.length === 0 ? (
                        <div className="py-12 text-center text-sm text-gray-500 dark:text-gray-400">No tasks in this project yet.</div>
                    ) : (
                        tasks.map(task => {
                            ...
                            return (
                                <div key={task.id} className="grid grid-cols-12 gap-4 px-4 py-3 items-center hover:bg-gray-50 dark:hover:bg-gray-800">
                                    <div className="col-span-4 flex items-center gap-2 min-w-0">
                                        ...
                                        <span className="text-sm font-medium text-gray-900 truncate dark:text-gray-100">{task.title}</span>
                                    </div>
                                    ...
                                    <div className="col-span-2 text-sm text-gray-700 dark:text-gray-300">{formatHoursHM(task.allocatedHoursNum)}</div>
                                    <div className={`col-span-2 text-sm font-medium ${running ? 'text-green-600 dark:text-green-400' : 'text-gray-700 dark:text-gray-300'}`}>
                                        {running ? formatTime(getElapsedTime(task.id)) : formatHoursHM(consumed)}
                                    </div>
                                    <div className="col-span-2 flex justify-end">
                                        {!isMine ? (
                                            <span className="text-xs text-gray-400 dark:text-gray-500">Not your task</span>
                                        ) : running ? (
                                            <button
